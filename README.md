...
**Upgrading from v3:** in v3, `fetch`, `fetchAndRefresh`, and the `semaphore` methods `consumeLock`, `expand`, `releaseLock`, `reset`, and `retrieveOrCreate` treated a missing callback as fire-and-forget and silently discarded errors. In v4 those same calls return a promise, and an error that was previously swallowed becomes an unhandled promise rejection — which terminates the process on Node.js 15 and later — unless the caller awaits the promise or attaches a `.catch()`. An awaited call is safe; the dangerous pattern is fire-and-forget — omitting the callback and discarding the returned promise. Every callback-less call written against v3 is that pattern, because these methods previously returned nothing to await. Audit those call sites before upgrading.
